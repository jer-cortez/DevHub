...
import { changesetSegments, pathToSegments } from '../lib/pathSegments';
...
import { repoContext } from './prSummary.services';
...
const AREA_SCHEMA = {
  ...
  properties: {
    overview: {
      type: 'string',
      description:
        'Three to five sentences explaining how this part of the codebase works: its ' +
    ...
    watch_for: {
      type: 'string',
      description:
        'Two to three sentences on what a reviewer unfamiliar with this area should scrutinise ' +
...
export interface RecentPr {
  ...
  author: string;
  merged_at: Date | null;
  /** Reuses the stored AI summary when the PR already has one. */
  summary: string | null;
}
...
export interface RelevantBoard {
  ...
  type: string;
  updated_at: Date;
  /** True when the board's title overlaps the concepts this PR touches. */
  matches_area: boolean;
}

export interface OnboardingView {
  /** False when the viewer has already worked in this repo — the UI hides itself. */
  isFirstTime: boolean;
  /** Needed by the client to build board links — the route keys on id, not name. */
  repo_id: string;
  repo_name: string;
  areaSegments: string[];
 ...
 */
async function hasWorkedInRepo(userId: string, repoId: string): Promise<boolean> {
  const [row] = await prisma.$queryRawUnsafe<{ worked: boolean }[]>(
    `
    select (
 ...
 */
async function relevantBoards(repoId: string, segments: Set<string>): Promise<RelevantBoard[]> {
  const boards = await prisma.drawing_boards.findMany({
  ...
  return boards
    .map((board) => ({
      ...board,
      matches_area: pathToSegments(board.title).some((token) => segments.has(token)),
    }))
    .sort((a, b) => Number(b.matches_area) - Number(a.matches_area))
    .slice(0, MAX_BOARDS);
}

async function recentlyMerged(repoId: string, excludePrId: string): Promise<RecentPr[]> {
  const prs = await prisma.pull_request.findMany({
...
async function areaSummary(
  ...
  const context = await repoContext(repoId);

  return cached(`onboarding:area:${repoId}:${areaKey}`, AREA_SUMMARY_TTL_SECONDS, async () => {
      ...
      system: [
        {
          type: 'text',
          text:
            'You orient engineers who are reviewing code in a repository they have never worked ' +
      ...
      messages: [
        {
          role: 'user',
          content:
            `A reviewer new to this repository is about to review a change touching: ` +
    ...
    return JSON.parse(block.text) as AreaSummary;
  });
}
...
export const OnboardingServices = {
   ...
   */
  async getForPr(prId: string, userId: string): Promise<OnboardingView> {
    ...
    const paths = pr.head_sha
      ? await PrDiffServices.listFilePaths(repo.name, pr.github_pr_number, pr.head_sha)
      : [];
    ...
    let summary: AreaSummary | null = null;
    try {
      summary = await areaSummary(
        pr.repo_id,
        segments,
        paths,
        recentPrs.map((p) => `#${p.github_pr_number} ${p.title}`)
      );
    } catch (error) {
