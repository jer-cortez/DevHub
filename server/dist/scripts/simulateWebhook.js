...
function buildPullRequest(repoFullName, action) {
...
function buildIssue(repoFullName, action) {
    const state = action === 'closed' ? 'closed' : 'open';
    return {
        id: FAKE_ISSUE_ID,
...
function buildComment(repoFullName, target, number) {
    return {
        id: FAKE_COMMENT_ID,
...
async function main() {
    ...
    const repo = repoName
        ? await prismaClient_1.prisma.repositories.findFirst({ where: { name: repoName } })
        : await prismaClient_1.prisma.repositories.findFirst();
    if (!repo)
        throw new Error(`No repository found${repoName ? ` named "${repoName}"` : ''}. Sync repos first.`);
    ...
    const signature = 'sha256=' + crypto_1.default.createHmac('sha256', WEBHOOK_SECRET).update(body).digest('hex');
    ...
    const created = await prismaClient_1.prisma.notifications.findMany({
