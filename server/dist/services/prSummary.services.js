...
exports.PrSummaryServices = void 0;
exports.repoContext = repoContext;
...
const SUMMARY_SCHEMA = {
    ...
    properties: {
        summary: {
            type: 'string',
            description: 'Two to four sentences on what this pull request changes, in plain ' +
                'language. Describe the behavior that changes, not which files were ' +
                'touched — the reader can see the file list.',
        },
        impact: {
            type: 'string',
            description: 'Two to three sentences on how this fits into the repository: which ' +
...
const SYSTEM_INSTRUCTIONS = 'You explain pull requests to engineers from other teams who do not know ' +
 ...
 */
async function repoContext(repoId) {
    ...
    let degraded = false;
    const context = await (0, cache_1.cached)(key, 86400, async () => {
        ...
        let paths = '';
        try {
            const { data: tree } = await github_1.octokit.rest.git.getTree({
                owner: ORG_NAME,
                repo: repo.name,
        ...
        let readme = '';
        try {
            const { data } = await github_1.octokit.rest.repos.getReadme({ owner: ORG_NAME, repo: repo.name });
    ...
    if (degraded)
        await (0, cache_1.invalidateCachePattern)(key);
    return context;
}
exports.PrSummaryServices = {
    /**
    ...
    async getOrGenerate(prId) {
        ...
        const [context, diff] = await Promise.all([
            ...
            prDiff_services_1.PrDiffServices.fetch(repo.name, pr.github_pr_number, pr.head_sha),
        ]);
        ...
        const response = await anthropic_1.anthropic.messages.create({
            ...
            messages: [
                {
                    role: 'user',
                    content: `Pull request #${pr.github_pr_number}: ${pr.title}\n` +
        ...
        console.log(`PR summary #${pr.github_pr_number} (${repo.name}): ` +
        ...
        return pullRequestSB_1.PullRequestSB.setSummary(pr.id, {
            summary: parsed.summary,
            summary_impact: parsed.impact,
            summary_sha: pr.head_sha,
            summary_model: anthropic_1.SUMMARY_MODEL,
