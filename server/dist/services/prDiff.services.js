...
exports.PrDiffServices = {
     ...
     */
    async listFilePaths(repoName, prNumber, headSha) {
        return (0, cache_1.cached)(`pr:filepaths:${repoName}:${prNumber}:${headSha}`, 86400, async () => {
            const files = await github_1.octokit.paginate(github_1.octokit.rest.pulls.listFiles, {
                owner: ORG_NAME,
                repo: repoName,
     ...
     */
    async fetch(repoName, prNumber, headSha) {
        return (0, cache_1.cached)(`pr:diff:${repoName}:${prNumber}:${headSha}`, 3600, async () => {
            const files = await github_1.octokit.paginate(github_1.octokit.rest.pulls.listFiles, {
                owner: ORG_NAME,
                repo: repoName,
                pull_number: prNumber,
